...
| **知识库** | 已上线 | 上传（md/txt/pdf ≤50MB）、快速捕获（文本/网页链接）、在线编辑、**标签 + 标题搜索**、删除/重建索引、**主-详阅读布局**（§4.1，v2.4 草图 design/kb-master-detail.html） |
...
- **阅读布局 = 主-详（v2.4，grilling 第五轮裁定，草图 design/kb-master-detail.html）**：
  - 文档列表收窄为左栏（标题 + 状态点 + 标签芯片 + 大小），点击即右侧详情面板阅读，**取消"新标签页打开原文"的默认行为**（收进面板操作"新窗口打开"）。
  - 详情渲染：Markdown 走渲染预览链（marked + DOMPurify）；PDF 走浏览器原生预览（iframe，引用跳页 `#page=N` 可用）。
  - 操作集中于面板头部：编辑（仅 Markdown）/ 标签 / 新窗口打开 / 删除；失败原因以红字显示在详情头部（窄列放不下）。
  - 进页面**默认无选中**，详情区显示占位引导「← 从左侧选择文档」。
  - 移动端（<760px）：列表 / 详情全屏二选一，详情左上角「← 返回列表」。
...
**v2.4 实施状态（2026-09-19/20）**：M-A（01 抓取端到端、02 URL 身份覆盖）✅；M-C（03 阵容编辑自重启、04 会话级换档重试生成）✅；M-B（05 评估 harness + 本地基线 ✅，06 调参）——05 的评估条目扩充至 ≥20 条需针对服务器真实语料标注（ADR 0008 门禁），06 有意保持阻塞。另：知识库主-详阅读布局已完成设计草图（design/kb-master-detail.html），实现排 v2.4 收尾。
...
- **v2.4.2（2026-09-20，grill-with-docs 更新）**：知识库阅读布局裁定固化进 §4.1（主-详 / 渲染方式 / 操作进面板头 / 默认无选中 / 移动端二选一；三个小默认：详情头红字显示失败原因、进页无选中占位、PDF 无编辑按钮）；§10 补 v2.4 实施状态（01–05 完成、06 门禁阻塞、主-详草图待实现）。
