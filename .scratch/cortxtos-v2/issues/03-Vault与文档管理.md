...
**验收清单**
- [x] 上传 md 与 pdf 各一个 → Vault 出现文件、列表出现行（状态 queued）
- [x] 同内容重复上传 → 不新增行（sha256 幂等）
- [x] 删除 → 文件与 DB 行同时消失，无孤儿
- [x] 在线编辑保存 → Vault 文件内容变更、状态回 queued
- [x] 51MB 文件 → 拒绝，错误信息明确

ready-for-agent: ✅ **已完成**

> 实现备注: 全部验收项由自动化测试 + 活体冒烟覆盖（commit f6f73e5 前一批）。真实 key 相关的终验项挪到票 08 现场过。
